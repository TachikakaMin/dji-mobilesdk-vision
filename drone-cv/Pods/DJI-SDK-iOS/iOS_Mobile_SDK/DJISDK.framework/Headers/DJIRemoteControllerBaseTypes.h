 ...
 *  the encode mode is single: Source 1 represents video from  one of the
 *  Source1Only input ports (HDMI or AV). Source 2 represents video from EXT input
 *  port  (HD Gimbal Camera).  When the encode mode is dual: Source 1 represents
 *  video from HDMI input port.  Source 2 represents video from AV input port.
 */
...
typedef NS_ENUM (uint8_t, DJIRCChargeMobileMode){
     ...
     *  then 20% and stops charging when the mobile device's battery is above 50%.  It
     *  is only supported by Inspire 2.
     */
...
typedef NS_ENUM(NSUInteger, DJIRCButtonAction) {
     ...
     *  200 V2 series.  For the other products, the action has to be implemented by the
     *  application itself.
     */
    ...
    /**
     *  Toggle AF and MF for camera. It is supported by the firmware of Matrice 200 V2
     *  series. For the other products,  the action has to be implemented by the
     *  application itself.
     */
     ...
     *  the  firmware of Matrice 200 series V2. For the other products, the action has
     *  to be  implemented by the application itself.
     */
     ...
     *  the firmware of Matrice 200 V2  series. For the other products, the action has
     *  to be implemented by the application itself.
     */
