 ...
 *  dedicated to the secondary video. The primary source only can  be
 *  `DJIVideoFeedPhysicalSourceLeftCamera` or `DJIVideoFeedPhysicalSourceFPVCamera`.
 *  The secondary source can be `DJIVideoFeedPhysicalSourceLeftCamera`,
 *  `DJIVideoFeedPhysicalSourceRightCamera`  and
 *  `DJIVideoFeedPhysicalSourceFPVCamera`. It is only supported by M200 series V2.
 *  
...
/**
 *  Assign the primary video and the secondary video source. The primary video
 *  source can be `DJIVideoFeedPhysicalSourceLeftCamera` or
 *  `DJIVideoFeedPhysicalSourceRightCamera`.  The secondary video source can be
 *  `DJIVideoFeedPhysicalSourceLeftCamera`, `DJIVideoFeedPhysicalSourceRightCamera`
 *  and `DJIVideoFeedPhysicalSourceFPVCamera`. The  primary video source and the
 *  secondary video source can't be same at the same time. Only supported by M210 V2
 *  series.
 *  
