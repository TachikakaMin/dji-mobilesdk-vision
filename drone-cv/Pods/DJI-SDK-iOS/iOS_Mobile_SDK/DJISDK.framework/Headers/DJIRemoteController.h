 ...
 *  remote controller module can connect to both an  FPV camera (through the HDMI
 *  and AV ports) and a camera mounted on the HD Gimbal (through the Gimbal port).
 *  The output video  can then be a combination of the two video sources. Either a
 *  single video source can be  displayed, or one can be displayed  within the other
 *  (as a Picture in Picture, or PIP). If the mode is set incorrectly,  then no
 *  output video will be displayed.  It is supported by Phantom 4 Professional,
 *  Matrice 200, Matrice 200 series V2, Inspire 1 and Inspire 2.
 *  
 ...
 *  source can be displayed, or  one can be displayed  within the other (as a
 *  Picture in Picture, or PIP). It is supported by Phantom 4 Professional, Matrice
